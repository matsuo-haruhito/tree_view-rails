...
The `changes` job classifies pull-request paths with `script/ci_changed_files_policy.mjs`. The main outputs are `docs_only`, `mockups_changed`, `browser_smoke_changed`, `package_sensitive`, `docker_setup_sensitive`, `docs_entrypoint_sensitive`, and `ci_policy_sensitive`. Keep `script/test_ci_changed_files_policy.mjs` in sync with any workflow or path-classification change so representative file sets continue to document the intended guard behavior.
