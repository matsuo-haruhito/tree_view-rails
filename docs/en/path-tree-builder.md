...
The same `row_partial` receives both generated folder nodes and record nodes. Branch on `node_type` when folder rows need generic metadata and record rows need host-app-specific columns or actions.
...
If you pass custom `folder_node_type:` or `record_node_type:` values, branch on those configured values instead of `:folder` and `:record`.
