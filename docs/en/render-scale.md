...
| Reduce child fetching | [Lazy Loading](lazy-loading.md) | Up-front child fetching and unloaded-child HTML | Host app implements fetch, queries, authorization, and responses. See the lazy-loading doc for a minimal controller and Turbo Stream pattern. |
| Page many children | [Children Pagination](children-pagination.md) | Children fetched per request | Host app owns cursor, offset, limit, next-page checks, and query strategy. See the children pagination doc for cursor and next-page examples. |
...
3. **Lazy loading**: Move here when fetching or preparing all descendants up front is the expensive part. The host app loads children for the toggled parent and returns the appropriate Turbo Stream response. Start with the copyable pattern in [Lazy Loading](lazy-loading.md).
4. **Children pagination**: Add this when a single parent can have many children and one expand request should return only a bounded page. Use the cursor, limit, and next-page examples in [Children Pagination](children-pagination.md).
...
TreeView renders child URL and row-state hooks. Fetch behavior, queries, pagination, authorization, and Turbo Stream responses are host app responsibilities. See [Lazy Loading](lazy-loading.md) for controller, Turbo Stream, loaded/error/retry, and authorization patterns.

## Children pagination

Use children pagination when a parent can have many children and the host app should fetch them in smaller pages.

TreeView does not choose cursor, offset, limit, ordering, next-page detection, or response shape. It provides integration boundaries through lazy-loading URLs and row data hooks. See [Children Pagination](children-pagination.md) for cursor-based pagination, limit clamping, stable ordering, next-page UI, and notes on selection or drag/drop with unloaded children.
