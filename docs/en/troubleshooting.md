...
- When the host app registers only some controllers or chooses a custom boot order, import `TreeViewControllerEntries` when you need the documented ordered `{ key, identifier, controller }` pairs, or `TreeViewControllerIdentifiers` when you only need the identifier strings.
