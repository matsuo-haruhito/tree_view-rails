...
- lazy loading と children pagination を使って、閉じた階層や次ページを host app 側で段階的に取得する
- 非常に深いデータをそのまま全展開する UI は避ける

極端な deep tree 専用の DOM 仮想化やスクロール制御は host app 側の責務とします。TreeView gem 側は表示範囲制御、lazy-loading hook、visible-row モデルの提供に留めます。

## large tree / performance hardening の扱い

TreeView gem 側では、深い・広いツリーを安全に扱うための土台として以下を提供します。

- `max_initial_depth` / `max_render_depth` / `max_leaf_distance` による描画量制御
- `expanded_keys` / `collapsed_keys` による初期展開制御
- `VisibleRows` による表示行の一次元化
- `lazy_loading` data 属性と remote-state hook
- `path_tree_for` / `reverse_tree_for` による注目ノード周辺への表示範囲縮小
- moderately deep / wide tree を対象にした baseline spec

一方で、以下は host app 側で実装・選択します。

- DB query の eager loading / pagination / cursor strategy
- search result や権限に応じた root_items の絞り込み
- 実DOMの virtual scroll / windowed renderer
- request throttling、debounce、retry、エラー表示の業務判断
- ブラウザごとの UX tuning

この境界により、TreeView gem はデータ量が増えたときの escape hatch を提供しつつ、host app のデータ取得戦略や画面UXを固定しません。
...
TreeView gem 側では、表示範囲制御、render state、stable DOM hook、lazy-loading hook、visible-row モデルのような土台を持ちます。CRUD、認証、ページング戦略、DOM 仮想スクロール本体、業務固有メッセージは host app 側に残します。
