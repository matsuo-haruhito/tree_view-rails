...
`tree_view_window` は helper option contract も `config/public_api_manifest.yml` で machine-readable に管理します。supported required keyword は `offset` と `limit` です。
...
package-root の JavaScript export、bundled controller identifier、helper method、helper option key の machine-readable な source of truth は `config/public_api_manifest.yml` に置きます。compatibility spec と entrypoint smoke check はその contract を参照して drift を検知します。
