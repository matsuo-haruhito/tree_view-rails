...
`tree-view-state:state-changed` も documented な `reason` value (`connect`, `refresh`, `expanded`, `collapsed`) を公開します。これらの文字列は event detail contract の一部ですが、独立した package-root value export ではありません。
