...
| `reason` | String | snapshot が公開された理由。`connect`, `refresh`, `expanded`, `collapsed` のいずれかです。host app は初期同期を保存対象から外したり、ユーザーによる expand/collapse だけを保存したりできます。 |

`reason` は snapshot の意味を変えません。`expandedKeys` は差分ではなく、引き続き現在の expanded-state snapshot 全体です。
...
| `sourceCheckbox` | HTMLInputElement or null | user toggle 由来の event では、変更元 checkbox。初回 connect、明示的な refresh、submit、その他 source のない path では `null`。 |
| `attemptedChecked` | Boolean or null | `sourceCheckbox` で試行された toggle 後の checked state。source のない path では `null`。 |

cascade selection が有効な場合も、`sourceCheckbox` はユーザーが操作した代表 checkbox だけを指します。descendant の delta list ではありません。結果としての selection snapshot は `selectedCount`、`selectedValues`、`selectedPayloads` を使ってください。

`sourceCheckbox` は `tree-view-selection:change` 専用です。`tree-view-selection:limit-exceeded` と `tree-view-selection:invalid-payload` の `checkbox` field は、それぞれの guard / error event に関係する checkbox を表します。
...
`TreeViewEventNames` は event 名、`TreeViewEventDetailKeys` は documented payload field 名、これらの value export は field に入る documented enum-like value を表します。`TreeViewStateChangeReasons` は state snapshot publish reason (`connect`, `refresh`, `expanded`, `collapsed`) に限定し、`TreeViewRemoteStateValues` は remote-state row value (`loading`, `loaded`, `error`) に限定し、`TreeViewTransferDropPositions` は transfer drop position (`before`, `inside`, `after`) に限定します。listener helper を追加したり、controller dispatch behavior を変更したりするものではありません。

raw な `tree-view-state:state-changed` の `event.detail.reason` string は、引き続き event detail contract の一部です。`TreeViewStateChangeReasons` は、それらの文字列を写経したくない host app の listener 分岐や test のための package-root value set にすぎません。

## 互換性方針

machine-readable public API manifest は、このページで文書化している event name、documented integration hook 名、代表的な必須 `event.detail` key、documented event value set を写して drift を検知するための guard です。一次の契約は引き続きこのページです。host app の test が documented detail key 名の machine-readable な一覧を必要とする場合は、event payload shape を変えずに package root の `TreeViewEventDetailKeys` を import できます。
