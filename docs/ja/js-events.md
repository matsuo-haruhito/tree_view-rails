...
host app 側の listener や test で machine-readable な参照先が必要な場合は、`tree_view/index.js` から `TreeViewEventNames` と `TreeViewEventDetailKeys` の両方を import してください。前者は event name string の写経を避けるため、後者は公開されている `event.detail` key 一覧を参照するための export です。
...
package-root export の `TreeViewEventNames` と `TreeViewEventDetailKeys` も、host app 側の listener / test から同じ documented contract を参照するための mirror です。
