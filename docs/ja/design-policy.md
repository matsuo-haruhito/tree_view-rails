...
## UI pattern を本体へ昇格する基準

新しい rendering DSL を追加する前に、まず cookbook として整理することを優先します。

host app partial や cookbook から TreeView 本体へ移すのは、以下を満たす場合に限定します。

- product-specific ではなく tree-specific である
- 複数の host app で有用である
- 薄い resolver、helper、configuration object として表現できる
- authorization、forms、modals、downloads、domain workflows と密結合しない

Column / Action rendering は、複数アプリで共通の TreeView-specific abstraction が見えるまでは、host app の row partial、helper、component、ViewComponent に残すことを推奨します。
