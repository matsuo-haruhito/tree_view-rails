...
空状態 wrapper の attribute と class 名は JavaScript package root の `TreeViewEmptyStateHooks` からも参照できます。host app や automated check は、markup を読み取ったり CSS selector を分解したりせず、documented hook 名を参照できます。
...
state controller の `keyboard` value が有効な場合、TreeView は軽量な row navigation helper を提供します。ArrowDown / ArrowUp は表示中の描画 row 間で focus を移し、ArrowRight / ArrowLeft は可能な場合に指定方向の expand/collapse を起動し、Enter / Space は row toggle を起動します。host app の interactive target から始まった event は無視するため、button、link、form control はそれぞれの keyboard behavior を維持できます。

この helper は完全な treegrid model より意図的に狭い範囲です。Home / End traversal、typeahead、roving tabindex の所有、page-level shortcut policy、host app column すべてが WAI-ARIA treegrid cell として振る舞うことまでは約束しません。
