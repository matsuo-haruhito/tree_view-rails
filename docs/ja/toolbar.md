...
toolbar action と state の対応は `config/public_api_manifest.yml` の `toolbar_actions` で管理します。この manifest は supported action name から、helper が `toggle_all_path` に渡す state value への mapping です。

`tree_view_toolbar_action_metadata` が返す metadata も `toolbar_action_metadata` で管理します。公開 key set は `action`、`state`、`label`、`path`、`disabled`、`data` です。TreeView が所有する `data` key は `tree_view_toolbar_action` と、disabled fallback control で使う `tree_view_toolbar_disabled` です。

`toggle_all_path` が使えない場合、metadata は `path: nil`、`disabled: true`、disabled data hook を返します。これにより host app 側の独自 toolbar markup は、route を捏造せず disabled fallback を描画できます。authorization、route availability、fallback copy の最終判断は引き続き host app 側の責務です。
...
`actions:`、`labels:`、`class_name:`、`button_class_name:`、`html:`、`action_html:` は `config/public_api_manifest.yml` の `tree_view_toolbar` helper option key set として追跡します。この option-key contract は、supported action name と `toggle_all_path` state value を追跡する `toolbar_actions` の action-to-state map、および返却 metadata hash shape を追跡する `toolbar_action_metadata` とは別の契約です。
