...
`actions:`、`labels:`、`class_name:`、`button_class_name:`、`html:`、`action_html:` は `config/public_api_manifest.yml` の `tree_view_toolbar` helper option key set として追跡します。この option-key contract は、supported action name と `toggle_all_path` state value を追跡する `toolbar_actions` の action-to-state map とは別の契約です。
