...
| 目的 | 入口 |
|---|---|
| 初めて TreeView を導入する | [導入手順](installation.md)、続けて [最小利用例](minimal-usage.md) と [使い方](usage.md) |
| 使う API や描画方式を選ぶ | [API判断ガイド](decision-guide.md)、続けて [API 概要](api-overview.md) と [API 仕様](api.md) |
| 大きな tree や一部描画を扱う | [描画スケール](render-scale.md)、[Lazy Loading](lazy-loading.md)、[Windowed Rendering](windowed-rendering.md)、[Children Pagination](children-pagination.md) |
| Selection、form、table layer とつなぐ | [Selection](selection.md)、[Form と編集行](form-editing.md)、[Resource table bridge](resource-table-bridge.md) |
| 公開契約や integration hook を確認する | [公開 API](public-api.md)、[JavaScript イベント契約](js-events.md)、[Host App 拡張ポイント](host-app-extension-points.md) |
| TreeView JavaScript controller を登録する | [Controller registration](controller-registration.md)。package-root surface は [公開 API](public-api.md#javascript-surface) も参照してください |
| root が空 / no-results row / empty copy の責務境界を確認する | [使い方: root items が空の場合 / no-results row](usage.md#root-items-が空の場合--no-results-row) と [empty-state.html](../mockups/empty-state.html) |
| 症状や責務境界から調べる | [トラブルシューティング](troubleshooting.md)、[FAQ](faq.md)、[Tree diagnostics](tree-diagnostics.md) |
...
| [Toolbar helper](toolbar.md) | `tree_view_toolbar`、expand/collapse action、`:current_path` contract、toolbar visual reference |
...
| [API判断ガイド](decision-guide.md) | use case から適切な API や option を選ぶための flowchart と対応表。GraphAdapter adapter mode を選ぶ場面も含みます。 |
...
| [Demo application boundary](demo-application-boundary.md) | static mockup と real Rails demo app の違い、demo repository が public になるまで直接 link しない方針 |
...
| [Cookbook](cookbook.md) | よく使う構成例と API の組み合わせ方。GraphAdapter と ActiveRecord の性能メモも含みます。 |
...
| [API 概要](api-overview.md) | records / resolver / GraphAdapter adapter mode を含む主要公開 API の概要 |
| [GraphAdapter](graph-adapter.md) | 1つの parent id column では扱いにくい異種nodeや graph-like node 向け adapter mode の dedicated guide |
...
| [ReverseTree](reverse-tree.md) | matched record を root として描画し、親方向 path をその下に表示する API |
...
| [Controller registration](controller-registration.md) | `registerTreeViewControllers(application)` による bundled Stimulus controller 登録と、custom boot flow 向け `TreeViewControllerEntries` |
...
| [Selection checkbox hooks](selection-checkbox-hooks.md) | 描画済み selection checkbox class と disabled reason attribute の package-root hook。selection behavior と送信値は Selection 本体を正本にします |
...
| [Persisted State](persisted-state.md) | generator と `StateStore` による開閉状態の保存 / 復元 / clear / prune。retention policy と cleanup schedule は host app が管理します |
| [Public Setup Surface](public-setup-surface.md) | persisted-state install generator の public setup contract、任意 owner 引数、生成先 path、host app review 境界 |
...
| [Direction-aware styling boundary](direction-aware-styling.md) | direction-aware な current-row、hierarchy connector、toggle spacing hook の host-app override guidance と将来条件 |
...
| ドキュメント | 内容 |
|---|---|
| Product Profile（repository-only） | repository の位置づけ、source-of-truth の順序、保守境界、non-goals。repository root で管理される保守者向け文書であり、packaged docs の entrypoint ではありません。 |
| AGENTS.md（repository-only） | repository 固有の maintainer workflow、最初に読む順序、docs 更新ルール。repository root で管理される保守者向け文書であり、packaged docs の entrypoint ではありません。 |
...
| [開発・保守方針](development.md) | test、CI、ドキュメント更新、現在の `npm ci` setup、lockfile-backed install policy |
| [CI policy suite](ci-policy-suite.md) | CI policy guard の targeted run、suite self-test、guard registration expectations |
| [Dependabot maintainer policy](dependabot-policy.md) | Bundler / GitHub Actions Dependabot lane、queue-size boundary、dependency triage scope |
| [JavaScript CI cache boundary](javascript-ci-cache-boundary.md) | `actions/setup-node` の npm cache scope、`npm ci`、package-lock source-of-truth guidance |
| [Dependabot Bundler 復旧手順](dependabot-bundler-recovery.md) | Dependabot Bundler PR の lockfile metadata drift を切り分け、範囲を絞って復旧する手順 |
...
大きな tree や一部描画を扱う場合は、まず [描画スケール](render-scale.md) で方針を選び、子ノードの必要時読み込み、表示行 window、server-side children pagination が必要になったら [Lazy Loading](lazy-loading.md)、[Windowed Rendering](windowed-rendering.md)、[Children Pagination](children-pagination.md) を確認してください。

tree 全体の expand/collapse 操作を置きたい場合は [Toolbar helper](toolbar.md) を参照してください。対応 action と `:current_path` contract の静的な visual companion として [toolbar-actions.html](../mockups/toolbar-actions.html) も使えます。
...
root items が空の場合や no-results row を確認したい場合は、baseline hook（`data-tree-view-empty-state="true"`、`.tree-view-empty-row__content`、`.tree-view-empty-row__message`）を [使い方: root items が空の場合 / no-results row](usage.md#root-items-が空の場合--no-results-row) で確認し、host app 側が決める empty copy、CTA、permission messaging、filter reset behavior の境界は [empty-state.html](../mockups/empty-state.html) で確認してください。

API 全体の入口を確認したい場合は、まず [API 概要](api-overview.md) を読み、細かい仕様は [API 仕様](api.md) を参照してください。

1つの parent id column では扱いにくい異種nodeや graph-like node の場合は、[API判断ガイド](decision-guide.md) から始め、必要に応じて [API概要: adapter mode](api-overview.md#adapter-mode)、[API仕様: TreeView::Tree](api.md#treeviewtree)、[Cookbook: GraphAdapter と ActiveRecord の性能](cookbook.md#graphadapter-と-activerecord-の性能) を確認してください。
...
matched record を root として見せ、親方向 path をその下に表示したい場合は [ReverseTree](reverse-tree.md) を参照してください。
...
Stimulus controller の通常登録や custom boot flow は [Controller registration](controller-registration.md) を参照してください。`registerTreeViewControllers(application)` を使う場面、`TreeViewControllerEntries` を参照する場面、host app が custom order / filtering を所有する境界を確認できます。
...
current-row cue、hierarchy connector、toggle spacing の host-app override と将来の direction-aware styling hook 条件は [Direction-aware styling boundary](direction-aware-styling.md) を参照してください。
