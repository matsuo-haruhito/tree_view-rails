...
Demo repository が public になるまでは、この boundary page を demo link 判断の durable handoff point として扱います。意図した入口は次のとおりです。

- root `README.md`: mockup / empty-state overview の近く
- root `docs/README.md`: static mockups と言語別 docs map の近く
- `docs/en/README.md` と `docs/ja/README.md`: 言語別の user-facing docs index entry

`docs/mockups/README.md` は static mockup review policy の source として扱います。この boundary へ読者を戻す導線は置けますが、demo app catalog にしたり private demo link を集めたりしないでください。release docs は shipped gem change の checklist として扱い、public docs navigation や publication status が実際に変わる release のときだけ demo app wording を追加してください。
