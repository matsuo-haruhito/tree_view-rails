...
この documented JavaScript lane は、entrypoint smoke check（`npm run test:entrypoints`）、Vitest unit tests（`npm test`）、Playwright browser smoke（`npm run test:browser`）をまとめて実行します。Pull Request CI と `main` push CI の両方でこの lane を使っており、JavaScript behavior、documented exports、browser interaction に触れる変更では同じ commands をローカルでも確認します。
...
- `docs/en/public-api.md` と `docs/ja/public-api.md`
- `docs/en/api.md` と `docs/ja/api.md`
- 高レベルのAPI案内や推奨 entry point に影響する場合は `docs/en/api-overview.md` と `docs/ja/api-overview.md`
- `CHANGELOG.md`
...
- PR CI で lint、Ruby specs、representative Rails checks、JavaScript checks が通るか
