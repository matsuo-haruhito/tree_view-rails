...
bulk edit row、per-row edit action、TreeView selection checkbox と host app 側 business control の境界を静的に見比べたい場合は [form-editing-rows.html](../mockups/form-editing-rows.html) を参照してください。

この mockup は、このページの責務整理を視覚的に補うためのものです。save workflow、Turbo response、authorization、persistence は引き続き host app 側の責務です。
