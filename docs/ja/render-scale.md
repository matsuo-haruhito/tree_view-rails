...
`TreeView::VisibleRows` は現在のrender stateに対するvisible row listをTreeView側でflattenする段階です。各 `TreeView::VisibleRows::Row` は manifest-backed row metadata reader として `item`, `depth`, `node_key`, `parent_key`, `has_children`, `expanded` を持ち、boolean predicate helper として `has_children?` / `expanded?` を持ちます。これらの値はTreeViewがすでに計算したvisible row listを説明するためのもので、server-side query scope、pagination controls、analytics interpretation、virtual scrolling strategyは引き続きhost app側の責務です。
...
host appがrender前またはwindow slicing前にflatten済みvisible row listを読む必要がある場合は、`TreeView::VisibleRows` を使います。publicな `VisibleRows::Row` metadata surface は、row identityと表示contextに必要な `item`, `depth`, `node_key`, `parent_key`, `has_children`, `expanded`, `has_children?`, `expanded?` に限定されます。

このrow metadataはdatabase query、server-side pagination、children page cursor、analytics grouping、scroll位置ベースのvirtualizationを決めません。また、visible rows計算後に `offset` / `limit` のslicing metadataを加える `TreeView::RenderWindow` とも責務を分けています。
