...
bundled HTML helperを使わず、host app側でtoolbar markupを組み立てたい場合は、まず documentedされた supported action set を取得してから、app-owned な link / button を描画します。
...
- `:state` 例: `:expanded`
- `:label`
- `:path`。現在のmodeがtree全体toggleを持たない場合は `nil`
- `:disabled`
- host app側のbutton / linkに使う `:data`

TreeView はmetadataだけを提供します。最終的なHTML構造、style、icon、authorization rule は host app側で決めます。
...
raw attribute 名も documented integration contract の一部ですが、host app 側の wiring や browser assertion では [Public API](public-api.md) にある `TreeViewIntegrationHooks.interactive.marker`、`TreeViewIntegrationHooks.interactive.ignoreKeyboard`、`TreeViewIntegrationHooks.interactive.ignoreRowClick`、`TreeViewIntegrationHooks.interactive.ignoreDrag` を使うと写経を避けられます。
