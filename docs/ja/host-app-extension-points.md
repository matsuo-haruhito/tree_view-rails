...
| expand / collapse / current path 外 collapse の toolbar action を描画する | `tree_view_toolbar`、`tree_view_toolbar_supported_actions`、`tree_view_toolbar_action_metadata`; TreeView は supported action metadata と data hook を持ち、host app は label、配置、route、toolbar 周辺の業務 policy を持つ | [Toolbar](toolbar.md)、[Public API](public-api.md#helper-methods) |
| current node path の breadcrumb navigation を描画する | `tree_view_breadcrumb`; TreeView は path helper output を描画し、host app は current item の選択、route target、authorization、業務固有 label を持つ | [Breadcrumb](breadcrumb.md)、[Localized names](localized-names.md) |
| expanded / collapsed / current tree state を request 間で保持する | `TreeView::PersistedState` と `TreeView::StateStore`; TreeView は normalized state object と store wrapper を持ち、host app は storage scope、lifecycle、request/session policy を持つ | [Persisted State](persisted-state.md)、[Public API](public-api.md#stable-public-entry-points) |
| 行の一部 window だけを描画する | `tree_view_window` と `TreeView::RenderWindow`; TreeView は rendered window helper と range validation を持ち、host app は pagination strategy、loading trigger、周辺 table / list behavior を持つ | [Windowed Rendering](windowed-rendering.md)、[Public API](public-api.md#helper-methods) |
