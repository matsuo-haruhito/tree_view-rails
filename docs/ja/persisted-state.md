...
- host app model経由で保存/読み込み/単一 tree instance のクリア/owner 単位のクリア/範囲指定 prune を行う `TreeView::StateStore`
...
1つの owner に紐づくすべての tree instance の保存状態をクリアする例:

```ruby
deleted_count = store.clear_owner!(owner: current_user)
```

`clear_owner!` は指定した owner の persisted-state record をすべて削除し、削除した row 数を返します。owner 単位の reset では単一の `tree_instance_key` がないため、`TreeView::PersistedState` は返しません。該当する record がない場合は `0` を返します。

owner-wide reset も storage helper に留まります。route、認可、確認 UI、retry、response shape、どのタイミングで owner 全体を reset するかという business rule は host app が所有します。`workspace:123:*` のような prefix-based reset が必要な場合は、TreeView の public helper surface を広げず、host app model または service layer 側の query / policy として扱ってください。
...
`StateStore#clear!` は、1つの owner と1つの `tree_instance_key` に対する reset です。`StateStore#clear_owner!` は、その owner のすべての tree instance を消す owner-wide reset helper で、削除した row 数だけを返します。`StateStore#prune!` は、明示した `older_than:` timestamp より古い row を消す opt-in cleanup helper です。
