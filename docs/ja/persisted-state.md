...
`StateStore#clear!` は、1つの owner と1つの `tree_instance_key` に対する reset です。retention policy、bulk cleanup helper、削除済み owner の prune task ではありません。

長く運用する host app では、persisted-state の lifecycle を host app 自身の storage policy として扱ってください。たとえば、古い行を期限切れにするか、削除済み owner の行を既存の dependent destroy や cleanup job で消すか、audit / privacy rule によって短い retention period が必要かは host app が決めます。

TreeView は現時点で cleanup rake task や default TTL を提供していません。必要な場合は、生成された model を使い、owner、`tree_instance_key`、timestamp、authorization rule に紐づく host app の scope として実装してください。将来 lifecycle helper を検討する場合も、これらの host-app policy 判断は current `find` / `save!` / `clear!` contract とは分けて扱います。
