...
特定の祖先だけ link にできない場合は、その item に対して `path_builder:` から `nil` を返してください。TreeView はその祖先を plain な `<span>` として描画し、link できる祖先、separator、現在 item の挙動は維持します。
...
これは item ごとの route 不在や認可上の link 不可を host app が明示するための逃げ道です。TreeView は crumb が link できない理由を判断せず、disabled link semantics や Turbo navigation behavior も追加しません。
...
`path_builder:` が non-current item に対して `nil` を返す場合、TreeView は `link_class:` class だけを持つ plain fallback `<span>` を使います。route-specific な data 属性や link semantics は実際の link だけのものなので、その fallback span には `link_html:` を適用しません。
...
| `path_builder:` | 各itemのURL/pathを返すcallable。省略時はplain label。itemごとの `nil` return は、その non-current crumb をplain labelとして描画します。 |
| `html:` | `<nav>` 要素への追加属性。 |
| `list_html:` | `<ol>` 要素への追加属性。 |
| `item_html:` | 各 `<li>` 要素への追加属性。callableにはitemが渡されます。 |
| `link_html:` | link要素への追加属性。callableにはitemが渡されます。`path_builder:` が non-current item に `nil` を返した場合の fallback label には適用されません。 |
...
| `link_class:` | link要素と non-current fallback label のclass。 |
...
| URL/path customization | builder hook, itemごとの `nil` fallbackを含む | provides routes and decides non-linkable items |
| lightweight HTML/data attributes | nav, list, item, link, current label, separator の merge hooks | provides attributes and behavior |
