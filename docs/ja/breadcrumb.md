...
`path_builder:` が non-current item に対して `nil` を返す場合、TreeView は fallback `<span>` に `link_class:` と `link_html:` hook を適用します。これにより、href なしの anchor を出さずに軽い data 属性を維持できます。
...
| `link_html:` | link要素への追加属性。callableにはitemが渡されます。`path_builder:` が non-current item に `nil` を返した場合の fallback `<span>` にも適用されます。 |
