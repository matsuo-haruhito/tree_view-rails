...
- docs-only CI は mockup inventory drift を自動検出しない。既存の mockup files と docs index がすでに食い違っている場合は、gallery redesign や mockup HTML/CSS 変更を checklist-only PR に混ぜず、別の docs follow-up として扱う。
