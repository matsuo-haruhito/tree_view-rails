...
`changes` job は `script/ci_changed_files_policy.mjs` で Pull Request の changed files を分類します。主な output は `docs_only`、`mockups_changed`、`browser_smoke_changed`、`package_sensitive`、`docker_setup_sensitive`、`docs_entrypoint_sensitive` です。workflow や path 分類を変える場合は、代表 file set が意図した guard behavior を説明し続けるように `script/test_ci_changed_files_policy.mjs` も同期してください。

`README.md`、`docs/**`、`Product Profile.md`、`CHANGELOG.md`、`AGENTS.md` だけに触れる docs-only Pull Request では、`lint` と `pr_specs` はそのまま残しつつ、representative Rails job を short-circuit します。branch protection のため、check 名はそのまま維持します。JavaScript job はさらに細かい path 分類で決まります。`README.md`、mockup 以外の `docs/**`、`CHANGELOG.md`、`config/public_api_manifest.yml` は `docs_entrypoint_sensitive` として `npm run test:docs-entrypoints` を実行します。`Product Profile.md` と `AGENTS.md` だけの場合は、package-facing docs signal ではなく repository-maintainer docs なので JavaScript を skip します。`docs/mockups/**` の変更は browser smoke path を実行します。`test/browser/**` の変更は docs-only shortcut の候補ではなく、browser smoke suite 自体を変えるため、明示的な browser smoke coverage も実行します。`.github/workflows/**` も変更する Pull Request では docs-only shortcut を使わず、通常の PR lanes を確認します。
