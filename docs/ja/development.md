...
Docs smoke / signal script を追加または rename した場合は、`node script/test_docs_entrypoint_suite.mjs --self-test` を実行し、suite registration が欠けていないことを確認します。この self-test は `npm run test:docs-entrypoints` 内の `Docs entrypoint suite option contract` group としても登録されています。candidate docs entrypoint script が suite の `checks` array または明示的な exclusion に入っているかを確認する registration guard であり、candidate pattern や CI workflow を再設計するための入口ではありません。
...
manifest structure の top-level sections、nested public JavaScript groups、event/detail shape、duplicate YAML keys だけを確認したい場合は `npm run test:public-api-manifest-structure` を使います。より広い `npm run test:docs-entrypoints` でも同じ manifest structure group を実行するため、この guard を変更するときは単体 command と suite 側の入口をそろえてください。
...
`changes` job は `script/ci_changed_files_policy.mjs` で Pull Request の changed files を分類します。主な output は `docs_only`、`mockups_changed`、`browser_smoke_changed`、`package_sensitive`、`docker_setup_sensitive`、`docs_entrypoint_sensitive`、`ci_policy_sensitive` です。workflow や path 分類を変える場合は、代表 file set が意図した guard behavior を説明し続けるように `script/test_ci_changed_files_policy.mjs` も同期してください。
