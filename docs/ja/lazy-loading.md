...
helper の `state:` は string-compatible な Ruby 引数のままです。TreeView はこれを `data-tree-remote-state` へ string coercion して pass-through し、closed enum として validation しません。ERB では host app 固有の placeholder state も含め、raw string を使い続けてかまいません。共有 JavaScript 側では、よく使う documented value 用の convenience surface として package root から `TreeViewRemoteStateValues.loaded` / `.error` を import できますが、この export は helper が受け付ける state の完全な一覧ではありません。
