...
| Accessibility, focus, or contrast cues | [keyboard-focus-states.html](keyboard-focus-states.html), [accessibility-semantics.html](accessibility-semantics.html), [high-contrast-state-cues/index.html](high-contrast-state-cues/index.html) | [keyboard-current-row/index.html](keyboard-current-row/index.html), [stylesheet-theme-boundary/index.html](stylesheet-theme-boundary/index.html), [reduced-motion-state-cues.html](reduced-motion-state-cues.html) |
...
| Toolbar action availability | [toolbar-actions.html](toolbar-actions.html) | [node-presenter-row-partials.html](node-presenter-row-partials.html), [path-tree-builder-rows.html](path-tree-builder-rows.html) |
| Empty, hidden-message, or table boundary questions | [empty-state.html](empty-state.html), [resource-table-empty-colspan-boundary.html](resource-table-empty-colspan-boundary.html) | [filtered-tree-modes.html](filtered-tree-modes.html), [table-caption-context.html](table-caption-context.html) |
...
| [resource-table-empty-colspan-boundary.html](resource-table-empty-colspan-boundary.html) | Focused resource-table empty-row reference showing broad `colspan="999"` fallback, host-owned exact colspan, selection/action columns, and TreeView empty-state wrapper boundaries. |
...
| [accessibility-semantics.html](accessibility-semantics.html) | Focused table-first accessibility semantics reference showing row-level ARIA placement, toggle `aria-expanded`, omitted `aria-controls`, and host-app-owned structure boundaries without promising a full treegrid model. |
...
| [stylesheet-theme-boundary/index.html](stylesheet-theme-boundary/index.html) | Focused theme-boundary reference comparing TreeView state cues across light, dark, and high-contrast-like host surfaces while keeping theme policy host-app owned. |
...
9. Use [resource-table-bridge.html](resource-table-bridge.html) when review needs a focused pass on table-owned columns plus TreeView-owned hierarchy cues; use [resource-table-empty-colspan-boundary.html](resource-table-empty-colspan-boundary.html) when the question is empty-row colspan ownership across selection, metadata, and action columns.
...
14. Use [accessibility-semantics.html](accessibility-semantics.html) when review needs to compare table-first row ARIA placement, toggle `aria-expanded`, omitted `aria-controls`, and host-app-owned page structure without treating TreeView as a full treegrid implementation.
15. Use [keyboard-current-row/index.html](keyboard-current-row/index.html) when review needs to compare focus-visible cues beside `aria-current="page"`, expanded/collapsed rows, and host-app-owned row actions in the same tree.
16. Use [high contrast state cues](high-contrast-state-cues/index.html) when review needs to compare current, selected, focus-visible, error/retry, and drop-target cues without relying on color alone.
17. Use [stylesheet theme boundary](stylesheet-theme-boundary/index.html) when review needs to compare reusable TreeView state cues across light, dark, and high-contrast-like host surfaces without promoting a public theme API.
18. Use [direction-aware cues](direction-aware-cues/index.html) when review needs to compare LTR baseline, RTL host-app override, and vertical writing stress cases without treating directional styling as a public hook contract.
19. Use [lazy-loading-handoff.html](lazy-loading-handoff.html) when review needs to isolate children container ownership and remote-state slot handoff from the broader interaction-state page.
20. Use [drop-positions.html](drop-positions.html) when review needs to compare before, inside, after, transfer disabled, and invalid transfer boundary cues without modeling host-app reorder rules or persistence.
21. Use [persisted-state-boundary.html](persisted-state-boundary.html) when review needs to compare persisted expansion state before, changed, restored, save-failed, retry, and cleanup retention cues without adding host-app persistence behavior, cleanup jobs, or retention policy.
22. Use [turbo-frame-target.html](turbo-frame-target.html) when review needs a focused pass on the configured `data-turbo-frame` link attribute and the host-app-owned frame target boundary.
23. Use [drag-interactive-controls.html](drag-interactive-controls.html) when review needs a focused pass on draggable rows that contain native controls, custom interactive markers, or drag-only ignore markers.
24. Use [interactive-marker-behaviors.html](interactive-marker-behaviors.html) when review needs to compare the broad interactive marker against the narrower keyboard, row-click, and drag behavior markers.
25. Use [windowed-rendering.html](windowed-rendering.html) when review needs to compare visible-row slicing, current-row anchoring, and first / last window boundary metadata without adding host-app paging controls.
26. Use [breadcrumb-paths.html](breadcrumb-paths.html) when review needs to compare breadcrumb-path context against the tree's own current-row cue without modeling host-app route design.
27. Use [filtered-tree-modes.html](filtered-tree-modes.html) when review needs to compare matched nodes against ancestor or descendant context without adding host-app search UI.
28. Use [path-tree-builder-rows.html](path-tree-builder-rows.html) when review needs to compare generated folder rows with record-backed rows without designing a file-manager application.
29. Use [node-presenter-row-partials.html](node-presenter-row-partials.html) when review needs to compare NodePresenter-provided row partial values against host-app-owned columns, permissions, and final actions.
30. Use [localized-row-labels.html](localized-row-labels.html) when review needs to compare long localized row labels, type badges, attribute labels, secondary metadata, and tooltip cues without deciding final host-app translations.
31. Use [form-editing-rows.html](form-editing-rows.html) when review needs to compare bulk edit rows, per-row edit action placement, or selection-versus-business checkbox roles without adding a real save workflow.
...
36. Use [empty-state.html](empty-state.html) when review needs a focused pass on no-root-items, no-results, or permission-hidden rows, the reusable empty-row wrapper hook, or the host-app-owned copy boundary.
...
- Use `accessibility-semantics.html` when the review question is row-level ARIA placement, toggle `aria-expanded`, omitted `aria-controls`, or the table-first / host-app-owned boundary rather than visual focus styling.
...
- Use the [stylesheet theme boundary](stylesheet-theme-boundary/index.html) focused subpage when the review question is whether TreeView state cues stay readable across light, dark, and high-contrast-like host themes without changing runtime stylesheet behavior.
