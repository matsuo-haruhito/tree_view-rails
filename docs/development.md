...
本体リポジトリの `docs/mockups/default-tree.html` は、gem標準のHTML構造・CSS適用状態・class / data属性の確認用です。

このモックで扱う範囲は以下です。

- `tree_view/tree_row` 周辺の代表的な table / row / cell 構造
- root / child / leaf / expanded / collapsed / hidden count の見え方
- selection checkbox、disabled checkbox、badge、marker、depth label、row actions の配置
- Railsを起動しなくてもレビューできる静的HTMLとしての確認性

実Rails appとしての挙動、controller、database、Turbo Stream応答、業務画面に近いplaygroundは demo repo 側で確認します。

## Betaリリース前の整理方針

beta版として公開する前に、APIと内部設計を整え、利用者向けdocsに未整理の作業予定を残さないことを優先します。
...
- `docs/installation.md`
  - 導入手順が変わった場合
- `docs/minimal-usage.md`
  - 最小利用例が変わった場合
- `docs/usage.md`
  - 使い方やサンプルが変わった場合
- `docs/cookbook.md`
  - 既存APIの組み合わせ例や実装パターンが増えた場合
- `docs/api.md`
...
- `docs/README.md`
  - 参照導線や保守者向け案内が変わった場合
- `CHANGELOG.md`
  - 利用者に見える変更、docs/example追加、整理内容を記録する場合
