...
test.describe("docs mockup browser smoke", () => {
  ...
  test("toolbar-actions.html preserves action and responsibility boundary signals", async ({ page }) => {
    ...
    await expect(page.getByText("Host app owns")).toBeVisible()
    await expect(page.getByText("Route generation, authorization policy, final permission copy, localization, and business-specific action names.")).toBeVisible()
    await expect(page.getByText("すべての部署とチームを展開")).toBeVisible()
    await expect(page.getByText("現在の申請ルートだけを表示")).toBeVisible()
  })
  ...
  test("path-tree-builder-rows.html preserves generated-folder and record-row boundary signals", async ({ page }) => {
    ...
    await expect(page.getByText("Generated folder row")).toBeVisible()
    await expect(page.getByText("Record row")).toBeVisible()
    await expect(page.getByText("Outside this mock")).toBeVisible()
    await expect(page.getByText("Routes, downloads, authorization copy, and final file-manager behavior stay outside the gem mockup.")).toBeVisible()
    ...
    await expect(page.getByText("FolderNode")).toBeVisible()
    await expect(page.getByText("RecordNode")).toBeVisible()
