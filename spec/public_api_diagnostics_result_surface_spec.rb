...
RSpec.describe "Public diagnostics Result surface" do
  ...
  it "keeps manifest-backed Result readers and helpers available" do
    result_surface.fetch("attributes").each do |attribute_name|
      expect(result).to respond_to(attribute_name), "expected Result##{attribute_name} to remain public"
    end

    result_surface.fetch("methods").each do |method_name|
      expect(result).to respond_to(method_name), "expected Result##{method_name} to remain public"
    end
  end

  it "keeps diagnostics docs aligned with the manifest-backed Result surface" do
    result_surface.fetch("attributes").concat(result_surface.fetch("methods")).each do |surface_name|
      diagnostics_docs.each do |relative_path, document|
        expect(document).to include("`#{surface_name}`"), "expected #{relative_path} to document Result##{surface_name}"
      end
    end
  end
end
